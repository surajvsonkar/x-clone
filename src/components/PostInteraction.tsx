	...
	const [optimisticCount, addOptimisticCount] = useOptimistic(
		...
		(prev, type: 'like' | 'rePosts' | 'save') => {
			...
			if (type === 'rePosts') {
				return {
					...prev,
					likes: prev.isReposted ? prev.reposts - 1 : prev.reposts + 1,
					isLiked: !prev.isReposted,
				};
			}
			if (type === 'save') {
				return {
		...
		<div className="flex items-center justify-between gap-4 lg:gap-16 my-2 text-textGray">
			<div className="flex items-center justify-between flex-1">
				<div className="flex items-center gap-2 cursor-pointer group">
					...
					<span className="text-sm group-hover:text-iconBlue">
						{optimisticCount.comments}
					</span>
				</div>
